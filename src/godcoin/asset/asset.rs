...
impl Asset {
    ...
    pub fn mul(&self, other: &Self, precision: u8) -> Option<Self> {
        ...
        let mul = (self.amount as i128).checked_mul(other.amount as i128)?;
        let final_mul = set_decimals_i128(mul, decimals, precision)?;
        if final_mul > ::std::i64::MAX as i128 { return None }
        Some(Asset {
    ...
    pub fn div(&self, other: &Self, precision: u8) -> Option<Self> {
        ...
        let decimals = cmp::max(cmp::max(self.decimals, other.decimals), precision);
    ...
    pub fn pow(&self, num: u16, precision: u8) -> Option<Self> {
        ...
        let decimals = (self.decimals as u16).checked_mul(num)?;
        ...
        res = set_decimals_big(&res, decimals, precision as u16);
        Some(Asset {
            amount: res.to_i64()?,
            decimals: precision,
            symbol: self.symbol
    ...
    pub fn to_str(&self) -> Box<str> {
        let mut s = String::from(self.amount.to_string());
        if self.decimals > 0 {
            let len = s.len();
