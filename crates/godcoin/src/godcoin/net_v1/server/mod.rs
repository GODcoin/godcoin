    ...
    let server = listener
        ...
        .for_each(move |socket| {
            ...
            let client = hs.and_then(move |(peer_type, frame)| {
                ...
                peer.get_sender()
                    .send_untracked(RpcPayload { id: 0, msg: None });
...
fn handle_peer(peer: Peer, blockchain: Arc<Blockchain>, minter: Arc<Option<Minter>>) {
    macro_rules! quick_send {
        ($sender:expr, $rpc:expr, $msg:expr) => {
            $sender.send_untracked(RpcPayload {
                id: $rpc.id,
                msg: Some($msg),
            });
        };
    ...
    let (tx, rx) = {
        ...
        (tx, rx.map_err(|_| Error::new(ErrorKind::Other, "rx error")))
    };
    ...
    tokio::spawn(
        ZipEither::new(peer, rx)
            .take_while({
                ...
                move |_| Ok(!force_close.load(Ordering::Acquire))
            })
            .for_each(move |(rpc, _)| {
                let rpc = match rpc {
                    Some(rpc) => rpc,
                    ...
                    RpcMsg::Block(var) => {
                        if let Some(height) = var.req() {
                            ...
                            quick_send!(
                                sender,
                                rpc,
                                RpcMsg::Block(Box::new(RpcVariant::Res(block)))
                            );
                        }
                    }
                    RpcMsg::Balance(var) => {
                            ...
                            match fee {
                                Some(fee) => {
                                    quick_send!(
                                        sender,
                                        rpc,
                                        RpcMsg::TotalFee(RpcVariant::Res(fee))
                                    );
                                }
                                None => {
