...
impl PeerPool {
    ...
    pub fn get_block(
        &self,
        height: u64,
    ) -> Option<impl Future<Item = Option<SignedBlock>, Error = ()>> {
        ...
        if let Some(peer) = peer {
            ...
            let fut = peer
                .tx
                .send(ClientEvent::Message(payload))
                .and_then(|rx| Ok(rx.unwrap()))
                .and_then(|rx| rx)
                .map(|payload| match payload.msg {
                    Some(msg) => match msg {
                    ...
                    match msg {
                        ...
                        RpcMsg::Properties(var) => {
                            if var.req().is_some() {
                                ...
                                quick_send!(state, id, RpcMsg::Properties(var))
                                    .wait()
                                    .unwrap();
                            }
                        }
                            ...
                            if let Some(addr) = var.req() {
                                ...
                                match fee {
                                    Some(fee) => {
                                        ...
                                        quick_send!(state, id, RpcMsg::TotalFee(var))
                                            .wait()
                                            .unwrap();
                                    }
                                    None => {
