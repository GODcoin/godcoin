            ...
            match msg_req {
                MsgRequest::GetBlock(height) => {
                    ...
                    match state.chain.get_block(height) {
                        Some(block) => {
                            MsgResponse::GetBlock(block.as_ref().clone()).into()
                        },
                        None => {
                            MsgResponse::Error(ErrorKind::InvalidHeight, None).into()
                        }
                    }
                }
            }
        ...
        Err(e) => {
            match e.kind() {
                _ => {
                    ...
                    MsgResponse::Error(ErrorKind::UnknownError, None).into()
                }
            }
        }
    }
...
fn main() {
    ...
    server::HttpServer::new(move || {
        App::with_state(AppState { chain: Arc::clone(&blockchain) })
